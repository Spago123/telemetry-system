...
#if !defined(CONFIG_LIB_TELEMETRY) || (CONFIG_LIB_TELEMETRY == 0)
...
static inline void send_messages_over_senders(const char *message, size_t len)
{
	for (size_t i = 0; i < registered_telemetry_senders.count; i++) {
		const struct telemetry_comm_interface *iface =
			registered_telemetry_senders.interfaces[i];
...
void telemetry_sender_task(void *a, void *b, void *c)
{
	...
	while (1) {
		...
		for (int i = 0; i < VAR_COUNT; i++) {
			...
			pos += snprintf(line + pos, sizeof(line) - pos, "%s:%g;", v->name,
					(double)*v->ptr);
			if (pos >= sizeof(line) - 32)
				break;
		}
		if (pos > 0) {
			line[pos++] = '\n';
